 ...
 *  hyphens, and capitalizes the first character — e.g. `ulb-wise-allocation-formula-template`
 *  becomes `Ulb-wise-allocation-formula-template`. A no-op for values already starting with a
 *  capital (state names from the DB) or a digit (year labels like `2024-25`), so this is safe to
 *  apply uniformly to every segment rather than special-casing `formName` alone. */
