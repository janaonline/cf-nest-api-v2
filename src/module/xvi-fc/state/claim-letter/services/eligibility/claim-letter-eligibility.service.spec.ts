...
describe('ClaimLetterEligibilityService', () => {
  ...
  describe('computeTotalAlreadyAcknowledged', () => {
    ...
    it('scopes the acknowledged-parent lookup to this state/year/installment and the acknowledged status', async () => {
      ...
      expect(filter['currentFormStatus']).toBe(FORM_STATUS.SUBMISSION_ACKNOWLEDGED_BY_MOHUA);
    });
  });

  describe('getFinancialOverview', () => {
    it('combines the state-wide allocation pool with the already-acknowledged total', async () => {
      ...
      expect(overview).toEqual({ totalInstallmentAllocation: 25, totalAlreadyAcknowledged: 5 });
    });

    it('returns zeros for a state that has never had a Devolution form or an acknowledged batch', async () => {
      devolutionFormModel.findOne.mockReturnValue(q(null));
      batchModel.find.mockReturnValue(q([]));

      const overview = await service.getFinancialOverview(stateId, designYearId, 1);

      expect(overview).toEqual({ totalInstallmentAllocation: 0, totalAlreadyAcknowledged: 0 });
    });
  });
});
