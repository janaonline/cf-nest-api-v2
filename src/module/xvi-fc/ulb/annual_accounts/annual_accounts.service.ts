...
/** Shared with AnnualAccountManualReviewService's email-notification copy. */
export const SECTION_LABELS: Record<AnnualAccountSectionKey, string> = {
  auditedData: 'Audited',
...
export class AnnualAccountsService implements OnModuleInit {
  ...
  async confirmUpload(
    ...
    await this.assertCanUlbUpload(dto.ulbId, dto.designYearId, dto.section as AnnualAccountSectionKey, dto.docId);
  ...
  async retryUpload(id: string, uploadId: string, user: AuthUser) {
    ...
    if (
      isAwaitingManualReviewDecision(
        docSlot?.currentUpload?.ocrInfo?.isManualReviewRequested,
        docSlot?.manualReviewDecision,
      )
    ) {
   ...
   *
   * Not private — AnnualAccountManualReviewService also resolves section documents ahead of a
   * manual-review request/decision, and reuses this instead of duplicating the lookup.
   */
  async resolveSectionDocument(
    id: string,
    section: AnnualAccountSectionKey,
   ...
   *
   * Not private — also called by AnnualAccountManualReviewService.
   */
  async validateViewAccess(doc: any, user: AuthUser): Promise<void> {
